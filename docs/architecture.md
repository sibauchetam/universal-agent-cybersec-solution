...
Харнесс скоупит ключи/модель/прокси по `--provider openrouter|groq` (дефолт
**openrouter**): ключ одного провайдера физически не может уйти в endpoint
другого (иначе — непрозрачный 401). Дефолты: openrouter — `qwen/qwen3.6-35b-a3b`
без прокси (endpoint доступен напрямую), groq — `qwen/qwen3.6-27b` через
SOCKS5 `GROQ_PROXY` (RPM 30, OTPM 1000, TPD 200k/ключ). Сам агент от провайдера
не зависит: endpoint/модель приходят через env от инфраструктуры заезда.
Замеченные Groq-специфические механизмы (пауза 65с на 413/ITPM, парсинг
«try again in Xs») на OpenRouter просто не срабатывают — деградации нет.
